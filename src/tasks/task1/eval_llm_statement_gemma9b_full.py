...
def main():
    """
    Task 1 full set:
    12 countries × 11 topics
    For each (country, topic), run 8 prompt variants.
    Each prompt asks for ratings over the full Schwartz value set.
    """
    ...
    # 先用 smoke test 跑 1 个 case，确认没问题再关掉
    smoke_test = False
    ...
    if smoke_test:
        ...
        total_cases = len(countries) * len(topics)
        print(f"Running full Task 1 set: {len(countries)} countries × {len(topics)} topics = {total_cases} cases")
    ...
    output_path = "/mmfs1/gscratch/cse/xunmei/value_action_gap/src/outputs/task1_gemma_statements_full.csv"
